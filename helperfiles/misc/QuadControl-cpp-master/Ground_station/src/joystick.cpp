	...
	while(true){
		...
		read(joy->fd, &joy->event, sizeof(struct js_event));
			...
			switch (joy->event.type & ~JS_EVENT_INIT)
			{
				...
				case JS_EVENT_AXIS:
					if(	abs((joy->axisOld[ joy->event.number])-(joy->event.value)) >= 5){
...
double joystick::IncrementVal_Button(double val,double rate, double lb, double ub){
	double c=val+rate;
	if (c>ub)
		c=ub;
...
double joystick::DecrementVal_Button(double val,double rate, double lb, double ub){
	double c=val-rate;
	if (c>ub)
		c=ub;
