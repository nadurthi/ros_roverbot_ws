...
double * quat_add(double * q,double * q1,double * q2){
	//double q[4];
	q[0]=q1[0]+q2[0];
	q[1]=q1[1]+q2[1];
	q[2]=q1[2]+q2[2];
...
double * quat_scalar_prod(double * q,double * q1,double c){
	//double q[4];
	q[0]=q1[0]*c;
	q[1]=q1[1]*c;
	q[2]=q1[2]*c;
...
double * NormQuat(double *q){
	double c=sqrt(pow(q[0],2)+pow(q[1],2)+pow(q[2],2)+pow(q[3],2));
	q[0]=q[0]/c;
	q[1]=q[1]/c;
	q[2]=q[2]/c;
	q[3]=q[3]/c;
...
void Euler2Quat(double * q,double  *Eul){
double phi,th,si;
phi=Eul[0];
th=Eul[1];
si=Eul[2];
q[0]=cos(phi/2)*cos(th/2)*cos(si/2)+sin(phi/2)*sin(th/2)*sin(si/2);
q[1]=sin(phi/2)*cos(th/2)*cos(si/2)-cos(phi/2)*sin(th/2)*sin(si/2);
q[2]=cos(phi/2)*sin(th/2)*cos(si/2)+sin(phi/2)*cos(th/2)*sin(si/2);
q[3]=cos(phi/2)*cos(th/2)*sin(si/2)-sin(phi/2)*sin(th/2)*cos(si/2);

}

double * Quat2Vec(double  *v ,double * q){
	v[0]=q[1];
	v[1]=q[2];
	v[2]=q[3];
...
double * Vec2Quat(double  *q ,double * v){
