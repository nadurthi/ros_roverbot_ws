...
void Control_angrate_shaping(QuadStateVariable *QuadStates){
	...
	// first compute the control for x,y,z error and total force F
     double ew[3],ex[3],ev[3],fd[3],R[3][3],b3d[3],b2d[3],nb1d[3],eq[4],qinv[4],fv[3],fu[3],c;
     ...
     c=sqrt(pow(fd[0],2)+pow(fd[1],2)+pow(fd[2],2));
     fu[0]=fd[0]/c;
       fu[1]=fd[1]/c;
       fu[2]=fd[2]/c;
       //cout<<"fang = "<<acos(fu[2])*180/3.14<<endl;
     ...
     Quat2Euler(Eulref ,QuadStates->qdes);//rot about z-y-x
 	//cout<<"qdes = "<<QuadStates->qdes[0]<<endl;
 	//cout<<"ex[0] = "<<ex[0]<<" ev[0] = "<<ev[0]<<endl;
    //cout<<"Eulref = "<<Eulref[0]*180/3.14<<" "<<Eulref[1]*180/3.14<<" "<<Eulref[2]*180/3.14<<endl;
     // required thrust



    //cout<<"fu = "<<fu[0]<<" "<<fu[1]<<" "<<fu[2]<<endl;
